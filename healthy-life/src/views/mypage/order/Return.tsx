...
function Return() {
  ...
  const getfetchData = async (orderDate: OrderGetRequestDto) => {
    try {
      ...
      const filterAllOrder = allOrder.filter((order:OrderDto) => order.orderDetails.some(order => order.orderStatus === "CANCELLED" || order.orderStatus === "RETURNED")); 
      setOrderDatas(filterAllOrder);
    } catch (error) {
      console.error(error);
    }
  };
      ...
      <div className="orderSearchContainer">
        ...
        <div className="orderDate">
          <input
            type="date"
            name="startOrderDate"
            value={orderDate.startOrderDate}
          ...
          <input
            type="date"
            name="endOrderDate"
            value={orderDate.endOrderDate}
            onChange={orderDateChange}
          />
        </div>
        <button
          type="submit"
          onClick={() => getfetchData(orderDate)}
          className="searchBtn"
        >
          조회
        </button>
