...
const Banner= () => {
  ...
  const fetchData = async() => {
    try {
      const response = await axios.get(`${MAIN_APT_PATH}${AUTH_PATH}${PRODUCT_PATH}${ALL_PRODUCTS}`);

      const responseData = response.data.data;

      const sortedBestItems = responseData.sort((a: ProductDetailResponseDto, b: ProductDetailResponseDto) => b.averageRating - a.averageRating);
